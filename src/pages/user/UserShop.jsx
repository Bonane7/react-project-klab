...
import { FiSearch, FiHeart, FiGrid, FiList, FiStar, FiShoppingBag } from "react-icons/fi";
        ...
        <div className={viewMode === "grid" 
          ...
          {filteredProducts.map((product, index) => {
            ...
            return (
              <div 
                key={productId} 
                className={`group bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-lg transition-all relative ${viewMode === "list" ? "flex gap-4 p-4" : ""}`}
              >
                <Link 
                  to={`/user/product/${productId}`}
                  className={`${viewMode === "list" ? "flex gap-4 w-full" : "block"}`}
                >
                  ...
                  <div className="p-4 flex-1">
                    ...
                    <p className="text-orange-500 font-bold mt-1 text-lg">
                      RWF {Number(productPrice).toFixed(2)}
                    </p>
                  </div>
                </Link>
                ...
                <button
                  ...
                  className={`absolute bottom-4 right-4 bg-orange-500 text-white p-2.5 rounded-full shadow-lg translate-y-12 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-500 hover:bg-orange-600 hover:scale-110 disabled:bg-gray-400 disabled:cursor-not-allowed ${
                    viewMode === "list" ? "bottom-6 right-6" : ""
                  }`}
                >
